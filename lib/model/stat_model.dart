...
class StatModel {
  ...
  StatModel.fromJson({required Map<String, dynamic> json}) :
    daegu = double.parse(json['daegu'] ?? '0'), //double.parse는 더블형태로 변경해주겠다는 의미
  ...
  static ItemCode parseItemCode(String raw) {
    ...
    // firstWhere는 조건에 맞는 첫번째 값을 반환해주는 함수
  }

  getLevelFromRegion(String region) {
    if (region == '서울') {
      return seoul;
    } else if (region == '경기') {
      return gyeonggi;
      ...
      throw Exception('알수없는 지역입니다. region: $region');
    }
  }
}